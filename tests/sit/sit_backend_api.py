...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--api-base",
        default=os.environ.get("IM_API_BASE", API_BASE),
        help="HTTP API base URL.",
    )
    parser.add_argument(
        "--ws-base",
        default=os.environ.get("IM_WS_BASE", IM_WS_BASE),
        help="Optional WebSocket base URL.",
    )
    parser.add_argument(
        "--mysql-container",
        default=os.environ.get("IM_MYSQL_CONTAINER", MYSQL_CONTAINER),
        help="MySQL container name for test data seeding.",
    )
    parser.add_argument(
        "--mysql-root-password",
        default=os.environ.get("MYSQL_ROOT_PASSWORD", MYSQL_ROOT_PASSWORD),
        help="MySQL root password.",
    )
    parser.add_argument(
        "--internal-secret",
        default=os.environ.get("IM_INTERNAL_SECRET", INTERNAL_SECRET),
...
def main() -> int:
    ...
    if IM_WS_BASE:
        ...
        print("SKIP websocket push: IM_WS_BASE is not configured")
        private_message = check(
            "send private message",
            lambda: alice.api(
                "POST",
    ...
    if IM_WS_BASE:
        ...
        print("SKIP websocket heartbeat: IM_WS_BASE is not configured")
    check(
        "delete account",
        lambda: carol.api(
            "DELETE",
