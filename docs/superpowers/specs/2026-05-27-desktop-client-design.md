...
│   ├── http.adapter.ts          # HttpClientPort → axios (绝对 URL，从环境变量读取)
...
│   ├── clock.adapter.ts         # ClockPort → 系统时钟
│   └── index.ts                 # 统一注册
├── public/
├── index.html
├── vite.config.ts
...
- `Ctrl+Alt+M` → 显示/隐藏主窗口
- `Ctrl+Alt+S` → 截屏功能（快捷键注册在第一阶段实现，截屏+发送的完整流程在后续迭代）
...
- Tauri v2 项目脚手架
- 6 个平台适配器（Storage、HTTP、Notifier、Lifecycle、Network、Clock）— Clock adapter 补充在项目结构中
- E2EE Rust 原生桥接
