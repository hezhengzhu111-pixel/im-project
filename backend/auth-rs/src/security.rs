...
use crate::jwt::sha256_base64_url;
...
pub fn validate_gateway_identity(
    headers: &HeaderMap,
    config: &AppConfig,
    ...
    let user_id_raw =
        header(headers, &config.gateway_user_id_header).ok_or_else(AppError::token_invalid)?;
    let user_id = user_id_raw
        .trim()
        .parse::<i64>()
        .map_err(|_| AppError::token_invalid())?;
    let username =
        header(headers, &config.gateway_username_header).ok_or_else(AppError::token_invalid)?;
...
mod tests {
    ...
    fn should_verify_internal_signature() {
        ...
        let body_hash = sha256_base64_url(body);
        let canonical = format!(
            "method=POST&path=/api/auth/internal/introspect&bodyHash={}&ts={}&nonce={}",
            body_hash, ts, nonce
        );
    ...
    fn should_reject_internal_signature_when_body_changes() {
        ...
        let ts = now_ms().to_string();
        let nonce = "n1";
        let body_hash = sha256_base64_url(b"token");
        let canonical = format!(
            "method=POST&path=/api/auth/internal/introspect&bodyHash={}&ts={}&nonce={}",
            body_hash, ts, nonce
        );
    ...
    #[test]
    fn should_verify_gateway_identity_signature() {
        let mut cfg = AppConfig::from_env();
        cfg.internal_secret = "internal".to_string();
        cfg.gateway_auth_secret = "gateway".to_string();
        let user = "eyJpZCI6N30";
        let perms = "WyJmaWxlOnJlYWQiXQ";
        let data = "e30";
        let ts = now_ms().to_string();
        let nonce = "nonce-1";
        let canonical = format!(
            "userId=7&username=alice&user={}&perms={}&data={}&ts={}&nonce={}",
            user, perms, data, ts, nonce
        );
        let sign = sign_hmac("gateway", &canonical);
        let mut headers = HeaderMap::new();
        headers.insert("X-Internal-Secret", HeaderValue::from_static("internal"));
        ...
        let identity = validate_gateway_identity(&headers, &cfg).unwrap();
        assert_eq!((7, "alice".to_string()), identity);
    }

    #[test]
    fn should_reject_gateway_identity_without_signature() {
        let mut cfg = AppConfig::from_env();
        cfg.internal_secret = "internal".to_string();
        let mut headers = HeaderMap::new();
        headers.insert("X-Internal-Secret", HeaderValue::from_static("internal"));
        ...
        assert!(validate_gateway_identity(&headers, &cfg).is_err());
    }
}
