...
mod ids_tests {
    ...
    fn ids_are_monotonically_increasing() {
        ...
        for _ in 0..100 {
            ...
            assert!(
                curr > prev,
                "ids must increase: prev={prev}, curr={curr}"
            );
            prev = curr;
        }
    }
    ...
    fn different_node_ids_produce_different_id_ranges() {
        let id_a = next_id(0);
        let id_b = next_id(1);
        assert_ne!(id_a, id_b, "different nodes should produce distinguishable ids");
    }
    ...
    fn node_id_is_reflected_in_id() {
        ...
        assert_eq!(extracted_node, 42, "node id should be encoded in bits 12-21");
    }
    ...
    fn node_id_clamped_to_10_bits() {
        ...
        let extracted_node = ((id >> 12) & 0x03ff) as u16;
        assert_eq!(extracted_node, 0x03ff, "node id should be masked to 10 bits");
    }
