...
mod time_tests {
    ...
    fn now_ms_returns_positive_value() {
        let ms = time::now_ms();
        assert!(ms > 0, "now_ms should return a positive timestamp, got {ms}");
    }
    ...
    fn now_iso_has_expected_format() {
        ...
        assert_eq!(iso.len(), 24, "ISO format should be 24 chars (with millis), got {} chars", iso.len());
    }
    ...
    fn iso_from_ms_zero_is_epoch() {
        ...
        assert!(iso.starts_with("1970-01-01"), "epoch must be 1970-01-01, got {iso}");
    }
