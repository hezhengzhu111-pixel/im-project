...
describe('enqueuePendingE2eeText', () => {
  ...
  it('enqueues pending with requiresE2ee, plaintext, and reason in payload', () => {
    ...
    enqueuePendingE2eeText(session, message, 'hello world', 'negotiation');
  ...
  it('uses the provided reason in e2eeWaitReason', () => {
    const session = { id: '100_200', type: 'private' as const, targetId: '200', targetName: 'Bob', unreadCount: 0, lastActiveTime: '', isPinned: false, isMuted: false } as ChatSession;
    ...
    enqueuePendingE2eeText(session, messageAlt, 'test', 'prekey');
...
describe('findPendingE2eeSends', () => {
  ...
  it('returns only pending items with requiresE2ee=true', () => {
    const e2eePending = makePending({ sendType: 'private', requiresE2ee: true, plaintext: 'secret', data: { receiverId: '200', clientMessageId: 'c1', messageType: 'TEXT' } });
  ...
  it('excludes non-pending status items', () => {
    const failed = makePending({ sendType: 'private', requiresE2ee: true, plaintext: 'secret', data: { receiverId: '200', clientMessageId: 'c1', messageType: 'TEXT' } }, { status: 'failed' });
...
describe('encryptPendingE2eePayload', () => {
  ...
  it('encrypts and updates pending payload on success', async () => {
    const item = makePending({ sendType: 'private', requiresE2ee: true, plaintext: 'hello secret', data: { receiverId: '200', clientMessageId: 'c1', messageType: 'TEXT' } });
  ...
  it('returns error when encryptToEnvelope fails', async () => {
    const item = makePending({ sendType: 'private', requiresE2ee: true, plaintext: 'secret', data: { receiverId: '200', clientMessageId: 'c1', messageType: 'TEXT' } });
  ...
  it('returns error for missing receiverId', async () => {
    const item = makePending({ sendType: 'private', requiresE2ee: true, plaintext: 'hello', data: { clientMessageId: 'c1', messageType: 'TEXT' } });
    const result = await encryptPendingE2eePayload(item);
    expect(result.ok).toBe(false);
