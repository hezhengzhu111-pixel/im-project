...
describe("Web session-store v3 envelope", () => {
  ...
  // ── saveSessionStateBytes defensive validation ────────────────────

  it("saveSessionStateBytes throws when remoteDeviceId is empty", async () => {
    await expect(
      saveSessionStateBytes(sessionId, mockState, {
        ...defaultMeta,
        remoteDeviceId: "",
      }),
  ...
  it("saveSessionStateBytes throws when localDeviceId is empty", async () => {
    await expect(
      saveSessionStateBytes(sessionId, mockState, {
        ...defaultMeta,
        localDeviceId: "",
      }),
    ).rejects.toThrow("E2EE session state requires localDeviceId");
  });

  it("saveSessionStateBytes throws when sessionId is empty", async () => {
    await expect(
      saveSessionStateBytes("", mockState, defaultMeta),
    ).rejects.toThrow("E2EE session state requires sessionId");
  });

  it("saveSessionStateBytes saves and restores with valid meta", async () => {
    await saveSessionStateBytes(sessionId, mockState, defaultMeta);
  ...
  // ── saveSessionStateBytes rejects remoteUserId="" when remoteDeviceId="" too ──

  it("saveSessionStateBytes throws when both remoteUserId and remoteDeviceId are empty", async () => {
    await expect(
      saveSessionStateBytes(sessionId, mockState, {
        localDeviceId,
        remoteUserId: "",
