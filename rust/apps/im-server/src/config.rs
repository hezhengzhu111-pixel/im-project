...
pub(crate) fn is_local_dev_or_test() -> bool {
    let value = env::var("APP_ENV").or_else(|_| env::var("IM_ENV")).ok();
    is_local_env_value(value.as_deref())
}

fn is_local_env_value(value: Option<&str>) -> bool {
    matches!(
        value
            .unwrap_or_default()
            .trim()
...
mod tests {
    ...
    fn is_local_dev_or_test_detection() {
        assert!(is_local_env_value(None));
        assert!(is_local_env_value(Some("")));
        assert!(is_local_env_value(Some("dev")));
        assert!(is_local_env_value(Some("test")));
        assert!(is_local_env_value(Some("local")));
        assert!(is_local_env_value(Some("development")));
        assert!(!is_local_env_value(Some("prod")));
        assert!(!is_local_env_value(Some("production")));
    }
}
