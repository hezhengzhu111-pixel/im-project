...
pub extern "C" fn session_manager_generate_pre_key_bundle(
    ...
    let mgr = unsafe { &*ptr };
    match mgr.generate_pre_key_bundle(
        signed_pre_key_id,
...
pub extern "C" fn session_manager_create_outbound_session(
    ...
    let result = mgr.create_outbound_session(sid.to_string(), ik.to_vec(), json.to_string());
...
#[unsafe(no_mangle)]
pub extern "C" fn session_manager_last_error(_ptr: *const SessionManager) -> *mut c_char {
    LAST_ERROR.with(|cell| {
