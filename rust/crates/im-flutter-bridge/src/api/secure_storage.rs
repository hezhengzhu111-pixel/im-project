...
mod native_impl {
    ...
    impl SecureKeyStore {
        ...
        pub fn get_master_key(&self) -> Result<Vec<u8>, String> {
            let entry =
                Entry::new(&self.service_name, "master_key").map_err(|e| format!("{}", e))?;
            Ok(entry
                .get_password()
                .map_err(|e| format!("{}", e))?
                .into_bytes())
...
mod native_impl {
    ...
    impl SecureKeyStore {
        ...
        pub fn set_master_key(&self, _key: &[u8]) -> Result<(), String> {
            Err("Not available in WASM".into())
        }
        pub fn encrypt(&self, _data: &[u8], _master_key: &[u8]) -> Result<Vec<u8>, String> {
            Err("Not available in WASM".into())
        }
        pub fn decrypt(&self, _ciphertext: &[u8], _master_key: &[u8]) -> Result<Vec<u8>, String> {
            Err("Not available in WASM".into())
        }
        pub fn delete_master_key(&self) -> Result<(), String> {
            Err("Not available in WASM".into())
...
pub use native_impl::{SecureBuffer, SecureKeyStore};
