...
fn decode_keypair(data: &[u8]) -> Result<X25519KeyPair, JsValue> {
    ...
    Err(JsValue::from_str(
        "invalid keypair: corrupted or unknown format",
    ))
}
...
fn to_js_error(e: im_e2ee_core::E2eeError) -> JsValue {
...
impl WasmSessionManager {
    ...
    pub fn create_outbound_session(
        ...
        let result = im_e2ee_core::x3dh_initiate(&ikp, &fetch).map_err(to_js_error)?;
    ...
    pub fn create_inbound_session(
        ...
        let result = im_e2ee_core::x3dh_respond_with_raw_otk(
            &ikp,
            &spkp,
            otk_pair.as_ref(),
            &remote_ik,
    ...
    pub fn decrypt(&mut self, session_id: String, encrypted: Vec<u8>) -> Result<Vec<u8>, JsValue> {
        ...
        let header: im_e2ee_core::RatchetHeader = decode_ratchet_header(header_bytes)
...
mod tests {
    ...
    fn encrypt_decrypt_roundtrip_wasm() {
        ...
        let bob_state_bytes = im_e2ee_core::try_export_state(&bob_state).expect("export bob state");
    ...
    fn wrong_session_decrypt_fails() {
        ...
        fn setup(mgr: &mut WasmSessionManager, alice_id: &str, bob_id: &str) {
            ...
            let bob_otk = bob_bundle.one_time_pre_key_pairs.first().expect("otk");
            let bob_otk_bincode = bincode::serialize(&bob_otk.key_pair).expect("serialize otk");
...
mod host_tests {
    ...
    fn encrypted_wire_format_header_and_ciphertext() -> Result<(), String> {
        ...
        if ciphertext_len < 16 {
            return Err(format!("ciphertext too short: {ciphertext_len} bytes",));
        }
